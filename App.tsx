...
function App() {
  // --- FUNCIÓN DE EXPORTACIÓN UNIVERSAL (MÓVIL Y PC) ---
  const handleExportMobile = async () => {
    try {
      const dataToExport = JSON.stringify(weightEntries, null, 2);
      const fileName = `pesos-backup-${new Date().toISOString().split('T')[0]}.json`;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <main className="flex-grow container mx-auto p-4 max-w-lg">
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <section className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
              <div className="flex justify-between items-center mb-4">
                ...
                <select value={filterPeriod} onChange={(e) => setFilterPeriod(e.target.value as FilterPeriod)} className="bg-indigo-50 border-none text-indigo-700 text-sm rounded-lg p-1 focus:ring-0">
        ...
        {activeView === 'history' && (
          <div className="animate-in fade-in duration-500">
             <WeightEntryList weightEntries={weightEntries} onEditRequest={setEditingEntry} onDelete={handleDeleteWeight} />
          </div>
        )}
        ...
        {activeView === 'settings' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <section className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
              ...
              <p className="text-xs text-slate-500 mb-4">Gestiona tus datos localmente o expórtalos para guardarlos.</p>
              <DataManagement 
      ...
      {error && (
        <div className="fixed top-20 right-4 left-4 bg-red-600 text-white px-4 py-3 rounded-lg shadow-xl z-50 animate-bounce text-center">
          {error}
        </div>
      )}

      {editingEntry && (
        <EditWeightModal entry={editingEntry} onSave={handleSaveEditedWeight} onClose={() => setEditingEntry(null)} />
      )}
    </div>
  );
}
...
const MenuButton = ({ active, onClick, label, icon }: { active: boolean, onClick: () => void, label: string, icon: React.ReactNode }) => (
  <button onClick={onClick} className={`flex items-center w-full p-4 rounded-xl transition-all duration-200 ${active ? 'bg-indigo-50 text-indigo-700 font-bold' : 'text-slate-600 hover:bg-slate-50'}`}>
